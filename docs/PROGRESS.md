...
> 최종 갱신: **2026-08-02** (sw.js **v610** / Phase 80~88 — CSS 문법 감사 · 스키마 드리프트 2차 · 홈 게스트 언어선택기 · 브랜드명 로마자 · **app_admins 보안 적용** · 관리자 지정 UI · 중복 ID · **`api/*.js` 전수 대조** · **무료재화 정리(쿠폰↔이용권 연결·신규가입 웰컴·추천 어뷰징)**. 앞서 Phase 75~79)
...
- 라이브 `baroalba-v610`, 작업트리 깨끗, `origin/main` 과 동기화됨
...
적립 성별은 `coupons.gender` → 없으면 `workers.gender` 순으로 정한다(성별은 가입 직후
필수 게이트에서 받으므로 사실상 항상 있다). 둘 다 없으면 **쿠폰을 소진시키지 않고** 400.

...
> 최종 갱신: **2026-08-01** (sw.js **v607** / Phase 80~83 — CSS 문법 감사 · 스키마 드리프트 2차(매니저 페이지 복구) · 홈 게스트 언어선택기 · 브랜드명 로마자 통일. 앞서 Phase 75~79)
...
- 라이브 `baroalba-v607`, 작업트리 깨끗, `origin/main` 과 동기화됨
...
### ③-B 🔴 지금 대표님이 제일 먼저 해야 할 것 — SQL 한 번 실행 (5분)

`docs/260728_fix_app_admins_rls.sql` 이 **작성만 되고 실행이 안 됐다.** 지금도 공개
anon 키로 관리자 이메일 명단이 읽히고, 아무나 자기 이메일을 관리자로 넣을 수 있다.
2장 단기 표 맨 위 항목 참고. **실행 후 관리자 로그인이 여전히 되는지 꼭 같이 확인할 것.**
...
| 항목 | 근거 | 해야 할 일 |
|------|------|-----------|
| **🔴🔴 `app_admins` 긴급 보안 SQL 이 실행된 적이 없다 — 대표님 5분 작업** | Phase 81(2026-08-01) 실측: 공개 anon 키로 `GET /rest/v1/app_admins` 가 **200 이고 관리자 이메일 3건이 그대로 읽힌다.** `is_app_admin()` 함수도 없어(PGRST202) `admin.html:1046` 은 매번 **폴백(테이블 직접 조회) 경로**로 돈다. 즉 `docs/260728_fix_app_admins_rls.sql` 이 "긴급"으로 적어둔 위험 — **아무나 가입 → 자기 이메일을 `app_admins` 에 INSERT → `/admin.html` 진입** — 이 1주일 넘게 그대로다 | **`docs/260728_fix_app_admins_rls.sql` 을 Supabase SQL Editor 에 붙여넣고 실행.** DDL 이라 대표님만 가능(규칙 4). 실행 후 확인: ①anon 으로 `app_admins` 조회가 401/404 가 되는지 ②관리자 계정으로 `/admin.html` 이 여전히 열리는지(함수가 생기면 폴백 대신 RPC 경로를 탄다). **②를 꼭 같이 볼 것** — 함수만 없고 테이블만 잠그면 관리자가 잠긴다 |

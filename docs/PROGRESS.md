...
> 최종 갱신: **2026-07-31** (sw.js **v600** / Phase 76 — 게스트가 언어를 되돌릴 방법이 없던 문제)
...
**남은 작업**: ①실기기 5화면(홈/지도/공고상세/마이페이지/업주 공고관리) 육안 확인
②3차 모서리(23종→4종)·그림자(82종→3종) — 그림자는 순검정 `rgba(0,0,0,…)`을
잉크색 `rgba(21,24,30,…)` 기반으로 바꾸는 게 핵심 ③4차 간격은 별도 스펙
④**`index.html`은 2차 치환 0건, `admin.html`은 2건** — 이 둘은 앱과 아예 다른
색 팔레트를 쓰고 있다는 뜻. `worker.html`은 `--red:#FF4B4B`로 **브랜드 레드
(#C8102E)와도 다르다.** 별도 정리 필요

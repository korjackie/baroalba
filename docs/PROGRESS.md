...
**아래 ①~⑥만 알면 바로 이어갈 수 있다. ⑥에 8/10 마감 시한이 하나 있다.**

### ① 지금 상태

- 라이브 `baroalba-v609`, 작업트리 깨끗, `origin/main` 과 동기화됨
...
- **바로스팟 무료 체험 종료일(8/10)** — 아래 ⑥ 참고. 연장/전환 결정이 필요하다.
...
| **바로스팟 무료 체험** | **바로스팟 1회 무료 참가**(`paid_method:'trial'`) | **코드 불필요.** 기간 안 + 바로스팟 신청 이력 0건이면 자동 | `assets/js/app.js` `BAROSPOT_TRIAL_EVENT` / `_isBarospotTrialEligible()` |

**가입만 하고 아무 코드도 안 넣으면 지급되는 건 없다.** 회원가입 축하 포인트·쿠폰은
존재하지 않는다(`point_accounts` INSERT 지점 4곳 전부 확인 — 추천보상·관리자 수동지급뿐).

⏰ **`BAROSPOT_TRIAL_EVENT.end` 가 `2026-08-10T23:59:59+09:00` 이다.** 그날이 지나면
신규 사용자는 **첫 신청부터 이용권 또는 2,000P** 를 내야 한다(1회권 가격, 상품표가 없을
때의 폴백값). 연장하려면 그 상수의 `end` 만 바꿔 배포하면 되고, 쿠폰으로 대체하려면
관리자 → 쿠폰 탭에서 코드를 발급하면 된다. **결정 필요.**

⚠️ 어뷰징 여지 하나: 추천 보상은 **계정당 1회 제한만** 있어서, 한 사람이 계정을 여러 개
만들어 서로 추천하면 계속 쌓인다. **3,000P 는 바로스팟 1회권 2,000P 보다 크다** — 즉
가짜 계정 하나로 무료 참가 1회가 나온다. 지금은 결제가 테스트 키라 실손해가 없지만,
**토스 라이브 키를 넣기 전에 정리하는 게 안전하다**(전화번호 인증·기기 단위 제한 등).

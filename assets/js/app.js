...
async function openCommunityPost(postId) {
  ...
  bindSheetDragClose(document.getElementById('comm-post-handle'), document.getElementById('comm-post-sheet'), closeCommunityPost);
...
// 바로 선점 여부를 물어본다(claimBarospotEvent와 동일한 원자적 선점 로직 재사용)
async function handleBarospotDeeplink(eventId) {
  openMannnamPanel();
...
async function submitWorkplaceVerify() {
  ...
  try {
    const r = await fetch('/api/admin?action=workplace_verify_send', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${session?.access_token || ''}` },
...
async function shareBarospotEventForApplicant(eventId) {
  ...
  const shareTitle = '🍽️ 바로스팟 남성 신청을 받고 있어요!';
  const descLine = `${name} · ${whenText}`;
  const shareText = `${shareTitle}\n${descLine}\n${link}`;

  if (/Android/i.test(navigator.userAgent) && window.AndroidBridge?.share) {
    window.AndroidBridge.share(shareTitle, shareText, link);
    return;
  }
  if (window.Kakao?.isInitialized?.()) {
    try {
      Kakao.Share.sendDefault({
        ...
        buttons: [{ title: '바로스팟 보러 가기', link: { mobileWebUrl: link, webUrl: link } }]
      });
      return;
    } catch (e) { /* SDK 오류 시 아래 공유 방식으로 폴백 */ }
  }
  if (navigator.share) {

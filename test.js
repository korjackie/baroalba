  ...
  (function() {
    ...
    if (/KAKAOTALK/i.test(ua)) {
      ...
      if (/Android/i.test(ua)) {
        // 앱(TWA) 설치 시 앱으로 열기, 미설치 시 Chrome 폴백
        var path = href.replace(/^https?:\/\//, '');
        location.replace(
          'intent://' + path +
          '#Intent;scheme=https;package=kr.co.multimove.baroalba;' +
          'S.browser_fallback_url=' + encodeURIComponent('https://play.google.com/store/apps/details?id=kr.co.multimove.baroalba') +
          ';end'
        );
      }
      // iOS KakaoTalk은 Safari 엔진 사용으로 대부분 정상 동작, 별도 처리 불필요
    }
  })();
  \n
  // document.documentElement.style.visibility='hidden';
  // 10초 안전 공개 — DOMContentLoaded가 loadJobs 완료 후 직접 취소/공개함
  window._headVizTimer=setTimeout(function(){document.documentElement.style.visibility='';},10000);
  // 버전 체크 → 구버전 캐시 리다이렉트
  (function(){
    var V='381',s=localStorage.getItem('_baroV'),u=new URLSearchParams(location.search).get('_v');
    if(s!==V&&u!==V){localStorage.setItem('_baroV',V);location.replace('/바로알바.html?_v='+V);}
  })();
...
db.auth.onAuthStateChange((event, session) => {
  if (event === 'SIGNED_OUT') { console.log('block'); return; }
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  if ('serviceWorker' in navigator) {
    navigator.serviceWorker.register('./sw.js?v=345').catch(()=>{});
...
function _moimHomeCard(m) {
  ...
  const dateStr = m.gathering_date ? new Date(m.gathering_date).toLocaleDateString('ko-KR',{month:'short',day:'numeric',weekday:'short'}) : '날짜 미정';
  const rem = (m.max_count||10) - (m.current_count||0);
  return `<div onclick="openMoimDetail('${m.id}')" style="flex-shrink:0;width:148px;background:#fff;border-radius:13px;overflow:hidden;cursor:pointer;border:1.5px solid #ede9fe">
    <div style="padding:10px 12px 8px;border-bottom:1px solid #f5f3ff">
      <div style="font-size:9px;font-weight:900;color:${color};letter-spacing:.5px;margin-bottom:3px">${cat.toUpperCase()}</div>
      <div style="font-size:12px;font-weight:800;color:#111;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;line-height:1.3">${m.title}</div>
    </div>
    <div style="padding:8px 12px">
      <div style="font-size:10px;color:#aaa">${dateStr}</div>
      <div style="font-size:11px;color:${rem<=2?'#C8102E':color};font-weight:700;margin-top:2px">${rem > 0 ? rem+'자리 남음' : '마감'}</div>
      ${m.entry_fee < 0 ? '<div style="font-size:11px;color:#D97706;font-weight:700;margin-top:1px">각자 분담</div>' : m.entry_fee > 0 ? `<div style="font-size:11px;color:#D97706;font-weight:700;margin-top:1px">${m.entry_fee.toLocaleString()}원</div>` : '<div style="font-size:11px;color:#16a34a;font-weight:700;margin-top:1px">무료</div>'}
    </div>
  </div>`;
}
...
function _renderChatList() {
  ...
  const _makeChatRow = (a) => {
    ...
    return `<div class="chat-swipe-wrap" id="csw-${a.id}">
      <div class="chat-swipe-inner" data-app-id="${a.id}" data-name="${a.counterpartName}" data-side="${a.side}" onclick="_chatItemClick(event,'${a.id}','${a.counterpartName}','${a.side}')" style="gap:12px;padding:11px 16px;background:${unread>0?'#FFFAFA':'#fff'};border-left:3.5px solid ${unread>0?'var(--red)':'transparent'}">
...
async function initOwnerFeatures() {
  ...
  applyLang();
  const { data: { session } } = await db.auth.getSession();
  if (!session) { console.log('block'); return; }
  currentUser = session.user;
...
function deleteOwnerAccount() {
  showConfirm('등록한 공고와 지원자 데이터가 모두 삭제됩니다.', () => {
    showConfirm('탈퇴 후 복구가 불가능합니다.', async () => {
      ...
      setTimeout(() => console.log('block'), 1800);
...
async function applyBaromeet(meetingId, maleLeft, femaleLeft) {
  ...
  // 성별 선택 후 포인트 차감
  showCustomConfirm({
    icon:'🤝', title:'바로미팅 참가 신청',
    msg:`참가 신청 시 2,000P가 차감됩니다.\n현재 잔액이 부족하면 충전 화면으로 이동합니다.`,
    okLabel:'2,000P 차감하고 신청', okColor:'#7C3AED',
    onOk: async () => {
      const pts = await loadUserPoints();
      if (pts < 2000) { closeMannnamPanel(); openPointCharge(); showToast('포인트가 부족해요. 충전 후 신청해주세요'); return; }
      showToast('신청이 접수됐어요! 확정 후 안내드려요');
    }
  });
}
...
async function applyBarospot() {
  ...
  const confirmed = await showCustomConfirm('바로스팟 신청', '이용권 1회가 차감됩니다.\n매니저가 제휴 식당과 일정을 배정해드립니다.\n신청하시겠어요?', '신청하기', '취소');
  if (!confirmed) return;
  const { data: passRow } = await db.from('barospot_passes')
...
async function applySpotEvent(eventId) {
  ...
  const confirmed = await showCustomConfirm('바로스팟 참가', '이용권 1회가 차감됩니다.\n참가 신청 후 매니저가 최종 확정합니다.\n신청하시겠어요?', '신청하기', '취소');
  if (!confirmed) return;
  const { data: passRow } = await db.from('barospot_passes')

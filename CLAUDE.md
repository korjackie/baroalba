...
## 0. 문서 지도 — 어느 문서를 믿을 것인가

이 프로젝트는 이력 문서가 여러 개다. **정본은 이 파일 하나뿐이다.**
다른 문서와 내용이 어긋나면 항상 이 파일이 이긴다. 새 세션은 여기부터 읽는다.

| 문서 | 역할 | 갱신 |
|------|------|------|
| **`CLAUDE.md`** (이 파일) | **정본.** 개발 규칙 + Phase별 작업이력 + 현재 버그 | 의미 있는 작업마다 **요청 없이도** 갱신 |
| `PROJECT.md` | 서비스가 *무엇인지* — 소개·기술구조·DB·타임라인 요약 | 구조가 바뀔 때만 |
| `docs/YYMMDD_changelog.md` | 그 시점 스냅샷. 쓰고 나면 고치지 않는 **읽기전용 기록** | 새로 만들지 말 것 (아래 참조) |
| `tools/i18n/README.md` | 번역 점검 도구 사용법 | 도구가 바뀔 때 |
| `backup/` | 옛 CLAUDE.md·PROJECT.md 사본. **참조 금지** (gitignore됨) | 안 함 |

저장소 밖(구글드라이브 `2. Projects/바로알바/`)에도 `PROGRESS.md`·`README.md`·
`milestone_*.md`·`기술전략/*.md` 가 있다. 전부 06월에 멈춘 **보존용 기록**이므로
사실 확인에 쓰지 말 것. 현재 상태가 궁금하면 이 파일이나 코드를 직접 볼 것.

**새 changelog 파일을 만들지 않는다.** 07-21 이후로 작업이력은 아래 7장 Phase 로그에
누적한다. 날짜별 파일이 늘어나면 어느 게 최신인지 매번 다시 판단해야 한다.
